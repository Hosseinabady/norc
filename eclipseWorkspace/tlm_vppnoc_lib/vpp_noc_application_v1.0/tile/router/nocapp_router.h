...
public:
  ...
  ( sc_module_name name
    ...
    for (int i = 0; i < NO_ROUTER_PORT; i++)
    {
     ...
     setLocation(NoCPosition(id%NO_OF_COL,id/NO_OF_COL));	
      ...
      while(1)
      {
        ...
        if (!*(owner->m_input_buffer[socket_id]->empty))
        {
          ...
          if (flit_type == FIRST_FLIT)
          {
            ...
            desPos.setX(dest_inx%NO_OF_COL);
            desPos.setY(dest_inx/NO_OF_COL);
            if ((desPos == owner->getLocation())) {
              int i;
              for (i = 0; i < NO_ROUTER_PORT; i++) 
                if (owner->m_switch[i][0] == true)
                  break;
              if ( i == NO_ROUTER_PORT) { // out port is free
                rtn = owner->m_outPort[0]->nb_transport_fw(*p_flit_trans, phase, delay);                
                if (rtn == VPPNOC_ACCEPTED) {
                  *(owner->m_output_port_free[0]) = false;
                  *(owner->m_output_port_ready[0]) = false;
                  owner->m_switch[socket_id][0] = true;
                  *(owner->m_input_buffer[socket_id]->head_update) = false;
                } else {
                  
                  *(owner->m_output_port_ready[0]) = false;
                  wait(flit_trans, delay, owner->m_output_port_ready[0], true, this);
                  cr_return(1); 
                }
              } else {
                wait(flit_trans, delay, owner->m_output_port_free[0], true, this);
                cr_return(1);
//                wait(flit_trans, delay, this);
//                cr_return(1);
//                cout << owner->name() << " Routing Error at function " << __FUNCTION__ << " at line " << __LINE__ << endl; 
              }
            } else if ( (desPos.getX() < owner->getLocation().getX()) ) {
              int i;
              for (i = 0; i < NO_ROUTER_PORT; i++) 
                if (owner->m_switch[i][3] == true)
                  break;
              if ( i == NO_ROUTER_PORT) { // out port is free
                rtn = owner->m_outPort[3]->nb_transport_fw(*p_flit_trans, phase, delay);
                if (rtn == VPPNOC_ACCEPTED) {
                  *(owner->m_output_port_free)[3] = false;
                  *(owner->m_output_port_ready[3]) = false;
                  owner->m_switch[socket_id][3] = true;
                  *(owner->m_input_buffer[socket_id]->head_update) = false;
                } else {
                  *(owner->m_output_port_ready[3]) = false;
                  wait(flit_trans, delay, owner->m_output_port_ready[3], true, this);
                  cr_return(1); 
                }
              } else {
                wait(flit_trans, delay, owner->m_output_port_free[3], true, this);
                cr_return(1);
//                wait(flit_trans, delay, this);
//                cr_return(1);
//                cout << owner->name() << " Routing Error at function " << __FUNCTION__ << " at line " << __LINE__ << endl; 
              ...
              int i;
              for (i = 0; i < NO_ROUTER_PORT; i++) 
                if (owner->m_switch[i][1] == true)
                  break;
              if ( i == NO_ROUTER_PORT) { // out port is free
                rtn = owner->m_outPort[1]->nb_transport_fw(*p_flit_trans, phase, delay);
                if (rtn == VPPNOC_ACCEPTED) {
                  *(owner->m_output_port_free)[1] = false;
                  *(owner->m_output_port_ready[1]) = false;
                  owner->m_switch[socket_id][1] = true;
                  *(owner->m_input_buffer[socket_id]->head_update) = false;
                } else {
                  *(owner->m_output_port_ready[1]) = false;
                  wait(flit_trans, delay, owner->m_output_port_ready[1], true, this);
                  cr_return(1); 
                }
              } else {
                wait(flit_trans, delay, owner->m_output_port_free[1], true, this);
                cr_return(1);
//                wait(flit_trans, delay, this);
//                cr_return(1);
//                cout << owner->name() << " Routing Error at function " << __FUNCTION__ << " at line " << __LINE__ << endl; 
              }
            } else if ( (desPos.getY() < owner->getLocation().getY())) {
              int i;
              for (i = 0; i < NO_ROUTER_PORT; i++) 
                if (owner->m_switch[i][4] == true)
                  break;
              if ( i == NO_ROUTER_PORT) { // out port is free
                rtn = owner->m_outPort[4]->nb_transport_fw(*p_flit_trans, phase, delay);
                if (rtn == VPPNOC_ACCEPTED) {
                  *(owner->m_output_port_free)[4] = false;
                  *(owner->m_output_port_ready[4]) = false;
                  owner->m_switch[socket_id][4] = true;
                  *(owner->m_input_buffer[socket_id]->head_update) = false;
                } else {
                  *(owner->m_output_port_ready[4]) = false;
                  wait(flit_trans, delay, owner->m_output_port_ready[4], true, this);
                  cr_return(1); 
                }
              } else {
                wait(flit_trans, delay, owner->m_output_port_free[4], true, this);
                cr_return(1);
//                wait(flit_trans, delay, this);
//                cr_return(1);
//                cout << owner->name() << " Routing Error at function " << __FUNCTION__ << " at line " << __LINE__ << endl; 
              ...
              int i;
              for (i = 0; i < NO_ROUTER_PORT; i++) 
                if (owner->m_switch[i][2] == true)
                  break;
              if ( i == NO_ROUTER_PORT) { // out port is free
                rtn = owner->m_outPort[2]->nb_transport_fw(*p_flit_trans, phase, delay);
                if (rtn == VPPNOC_ACCEPTED) {
                  *(owner->m_output_port_free)[2] = false;
                  *(owner->m_output_port_ready[2]) = false;
                  owner->m_switch[socket_id][2] = true;
                  *(owner->m_input_buffer[socket_id]->head_update) = false;
                } else {
                  *(owner->m_output_port_ready[2]) = false;
                  wait(flit_trans, delay, owner->m_output_port_ready[2], true, this);
                  cr_return(1); 
                }
              } else {
                wait(flit_trans, delay, owner->m_output_port_free[2], true, this);
                cr_return(1);
//                wait(flit_trans, delay, this);
//                cr_return(1);
//                cout << owner->name() << " Routing Error at function " << __FUNCTION__ << " at line " << __LINE__ << endl; 
