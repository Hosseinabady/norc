...
/*
	...
	virtual vppnoc::sync_enum_type upstream_nb_transport_bw(int initiator_id, vppnoc::transaction_type& r_response_packet_trans, vppnoc::phase_type& phase, sc_core::sc_time& t) {
#ifdef DEBUG
		trafficLogFile << name() << " upstream_nb_transport_bw PORT = " << initiator_id << std::endl;
#endif
//		r_response_packet_trans.acquire();

		if (phase == vppnoc::BEGIN_RESP) {
			m_ni_manager->notify((TransportLayer*)this, initiator_id, r_response_packet_trans, t, (int*)NULL, initiator_id, m_action_label[TRANSPORT_UP_STREAM_BW]);
			phase = vppnoc::END_RESP;
			t = sc_core::SC_ZERO_TIME;
			r_response_packet_trans.set_response_status(vppnoc::VPP_NOC_OK_RESPONSE);
			return vppnoc::VPP_NOC_UPDATED;
		} else if (phase == vppnoc::END_RESP) {
	    ...
	    return vppnoc::VPP_NOC_COMPLETED;  //unreachable code
	}




	virtual vppnoc::sync_enum_type nb_external_transport_fw(vppnoc::NoCApplication* app) {
		// EXTERNAL TASK REQUEST
#ifdef DEBUG
		outputLogFile << "Third Layer function call " <<  " tile_id= " << tile_id << std::endl;
#endif
		TaskManager* tsk = new TaskManager();
		tsk->setTaskId(0);
		tsk->setTaskType(-1);
		tsk->setFileName(app->getFileName());
		tsk->setApplicationId(app->getId());
		
		tsk->setNominalPeriod(app->getNominalPeriod());
		if (app->getState() == 0) {  //Task is not mapped
			Volunteer* vl = new Volunteer;
			Reconfiguration(tsk, vl);
			if (vl->core_id != -1) { 
				app->setTaskManagerTileId(getId());
				app->setTaskManagerCoreId(vl->core_id);

				tsk->setHostTileId(getId());
				tsk->setHostCoreId(vl->core_id);
				reconfigurable_area[vl->core_id].activity = CORE_ACTIVE;
				reconfigurable_area[vl->core_id].task     = tsk;
				vppnoc::vppnoc_generic_payload* p_message_trans;
    			p_message_trans = m_mm.allocate();
			//	p_message_trans->acquire();
			
				message_payload* p_message_trans_ext;
				p_message_trans_ext = new message_payload;
		
				//p_message_trans->set_extension(p_message_trans_ext);

				noc_gp_hdr(p_message_trans_ext, m_cmd) = TASK_ACTIVE_REQ;
				app->setState(1);
				...
				noc_gp_hdr(p_message_trans_ext, m_dst_tile)    = app->getTaskManagerTileId();
				noc_gp_hdr(p_message_trans_ext, m_dst_core)    = app->getTaskManagerCoreId();
				noc_gp_hdr(p_message_trans_ext, m_src_core)    = app->getTaskManagerCoreId();
						

				sc_core::sc_time           t(sc_core::SC_ZERO_TIME);
				vppnoc::phase_type phase = vppnoc::BEGIN_REQ ;
				p_message_trans->set_address((sc_dt::uint64)(app->getTaskManagerCoreId()+1));
				//std::cout << "Task Manager Tile id =" << tile_id << std::endl;
				//std::cout << "Task Manager Core id =" << app->getTaskManagerCoreId() << std::endl;
				switch (up_layer->upstream_nb_transport_fw(0, *p_message_trans, phase, t)) {
				case vppnoc::VPP_NOC_COMPLETED:
				case vppnoc::VPP_NOC_UPDATED:
					p_message_trans->release();
					break;
				case vppnoc::VPP_NOC_ACCEPTED:
					UNDER_CONSTRUCTION();
					break;
				default:
				...
				delete tsk;
				app->setTaskManagerTileId(-1);
				app->setTaskManagerCoreId(-1);
#ifdef TASK_MAPPING_PATTERN_LOG
			taskMappingPatternLogFile << "    <Application  Id=\"" <<  app->getId() << "\"  Status=\"" << (app->getState()? "attempt to map":"attempt to release") << "\">" << std::endl;
			taskMappingPatternLogFile << "        <Task  Id=\"0\">" << std::endl;
			taskMappingPatternLogFile << "            <Location CoreId=\"" << app->getTaskManagerCoreId() << "\"  TileId=\"" << app->getTaskManagerTileId() << "\" Status=\"" << (app->getState()? "mapped":"released") << "\"/>" << std::endl;
			...
			taskMappingPatternLogFile << "        </Task>" <<  std::endl;
				...
				return vppnoc::VPP_NOC_UPDATED;  // task manager cannot be mapped
			}
			delete vl;
#ifdef TASK_MAPPING_PATTERN_LOG
			taskMappingPatternLogFile << "    <Application  Id=\"" <<  app->getId() << "\"  Status=\"" << (app->getState()? "attempt to map":"attempt to release") << "\">" << std::endl;
			...
			return vppnoc::TLM_ACCEPTED;  
		} else {
			vppnoc::tlm_generic_payload* p_message_trans;
    		p_message_trans = m_mm.allocate();
		//	p_message_trans->acquire();
		
			vppnoc::noc_message_payload* p_message_trans_ext;
			p_message_trans_ext = new tlm_noc::noc_message_payload;
		
			p_message_trans->set_extension(p_message_trans_ext);

			noc_gp_hdr(p_message_trans_ext, m_cmd)     = vppnoc::TASK_RLS_ACK;
			app->setState(0);
			...
			switch (downstream_nb_transport_fw(0, *p_message_trans, phase, t)) {
			case vppnoc::TLM_COMPLETED:
			case vppnoc::TLM_UPDATED:
				p_message_trans->release();
				break;
			case vppnoc::TLM_ACCEPTED:
				UNDER_CONSTRUCTION();
				break;
			default:
				std::cout << "Error: '" << name() << "': Illegal phase received from initiator." << std::endl;
