...
ZigzagTask::ZigzagTask() {

}

int ZigzagTask::action(tlm::tlm_generic_payload* p_trans) {
  tlm_noc::noc_message_payload* p_message_trans_ext;
  p_trans->get_extension(p_message_trans_ext);
	std::cout << "I'm Zigzag " << sc_core::sc_time_stamp() <<  "turn = " << noc_gp_hdr(p_message_trans_ext, m_message_id) << std::endl;
  //---------FUNCTION START----------------------------

  DATA_TYPE tmp[64];
  int i, j;
  OutputFile.open("zigzagOut.txt");
  OutputFile2.open("zigzagIn.txt");
