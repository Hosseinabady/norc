...
QuantizerTask::QuantizerTask() {
  init();
}

int QuantizerTask::action(tlm::tlm_generic_payload* p_trans) {
  tlm_noc::noc_message_payload* p_message_trans_ext;
  p_trans->get_extension(p_message_trans_ext);
	std::cout << "I'm Quantizer " << sc_core::sc_time_stamp() <<  "turn = " << noc_gp_hdr(p_message_trans_ext, m_message_id) << std::endl;
  //---------FUNCTION START----------------------------

  OutputFile.open("QuantizerOut.txt");
  OutputFile2.open("QuantizerIn.txt");
  int i;
