...
int ConverterTask::action(tlm::tlm_generic_payload* p_trans) {
  tlm_noc::noc_message_payload* p_message_trans_ext;
  p_trans->get_extension(p_message_trans_ext);
	std::cout << "I'm converter " << sc_core::sc_time_stamp() <<  "turn = " << noc_gp_hdr(p_message_trans_ext, m_message_id) << std::endl;
  //---------FUNCTION START----------------------------
  int i;
  turn++;
  if ((turn % 2) == 0) 
	OutputFile.open("ConvertingIn1.txt");
  else 
	  OutputFile.open("ConvertingIn2.txt");
  ...
  OutputFile.close();
	...
	//noc_gp_hdr(p_message_trans_ext, m_message_id)  = turn+200;
