...
int BlockingTask::action(tlm::tlm_generic_payload* p_trans) {
  tlm_noc::noc_message_payload* p_message_trans_ext;
  p_trans->get_extension(p_message_trans_ext);

	std::cout << "I'm Blocking " << sc_core::sc_time_stamp() << "turn = " << noc_gp_hdr(p_message_trans_ext, m_message_id) << std::endl;
  //---------FUNCTION START----------------------------
  int i = 0;
  OutputFile.open("BlockingOut.txt");
  OutputFile2.open("BlockingIn.txt");

  int message_payload_size = noc_gp_hdr(p_message_trans_ext, m_payload_size);
  for (i = 0; i < message_payload_size; i++)
	  OutputFile2 << p_message_trans_ext->noc_trans.view_as.pdu.body[i];
  OutputFile2.close();
