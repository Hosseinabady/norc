...
class NoCRouterManager :public NoCAbstractModule {
public:
...
public:
	...
	NoCRouterManager(sc_core::sc_module_name name, OWNER* _owner, int id) : NoCAbstractModule(name, id), m_owner(_owner), number_of_cbfunction(0)
	{
	...
	void notify (tlm_payload_type& t, tlm_phase_type& p, const sc_core::sc_time& when, const int& channel, const unsigned int& part) {
		if (when == sc_core::SC_ZERO_TIME) {
			if (sc_core::sc_delta_count() & (sc_dt::uint64)0x1)
				m_even_delta.insert(PAYLOAD(&t, p, channel, part));
			else
				m_uneven_delta.insert(PAYLOAD(&t, p, channel, part));
			m_e.notify(sc_core::SC_ZERO_TIME);
		} else {
			m_ppq.insert(PAYLOAD(&t, p, channel, part), when+sc_core::sc_time_stamp());
			m_e.notify();
		}
	}

	void notify (tlm_payload_type& t, tlm_phase_type& p, const int& channel, const unsigned int& part) {
		m_immediate_yield.insert(PAYLOAD(&t, p, channel, part));
		m_e.notify();
	}
	void notify (tlm_payload_type& t, const int& channel, const unsigned int& part) {
		if (part == 0) {
			m_wait_list.insert(PAYLOAD(&t, 0, channel, part));
		} else if (part == 1) {
